...
def seed_if_empty(conn: sqlite3.Connection) -> None:
    ...
    conn.execute("""INSERT INTO sms_announcements (id, school_id, title, content, target_audience, priority, is_published, created_at)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
        ("ANN-1-" + school_id, school_id, "Welcome to 2025-2026 Academic Year", "We are excited to welcome all students to the new academic year. Classes begin on September 1, 2025.", "all", "high", 1, today))
    conn.execute("""INSERT INTO sms_announcements (id, school_id, title, content, target_audience, priority, is_published, created_at)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
        ("ANN-2-" + school_id, school_id, "Course Registration Open", "Course registration for First Term is now open. Please register before the deadline.", "students", "normal", 1, today))
    conn.execute("""INSERT INTO sms_announcements (id, school_id, title, content, target_audience, priority, is_published, created_at)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
        ("ANN-3-" + school_id, school_id, "Mid-Term Examinations", "Mid-term examinations will be held from November 15-20, 2025.", "students", "normal", 1, today))
...
def serialize_school(row: sqlite3.Row) -> dict:
    return {
        ...
        "country": _row_get(row, "country") or "Ghana",
        "tagline": _row_get(row, "tagline")
    }
...
def serialize_staff(row: sqlite3.Row) -> dict:
    return {
        ...
        "gender": _row_get(row, "gender"),
        "qualification": _row_get(row, "qualification"),
...
def serialize_student(row: sqlite3.Row, include_password: bool = False) -> dict:
    payload = {
        ...
        "status": row["status"],
        "gender": _row_get(row, "gender"),
        "guardianName": _row_get(row, "guardian_name"),
        "guardianPhone": _row_get(row, "guardian_phone"),
...
def serialize_data(conn: sqlite3.Connection, school: sqlite3.Row) -> dict:
    ...
    for row in conn.execute(
        ...
        attendance.append({
            ...
            "takenBy": _row_get(row, "taken_by")
        })

    # Build student name lookup for enriching exam results
    ...
    exam_results = []
    for row in conn.execute(
        "SELECT * FROM sms_exam_results WHERE school_id = ? ORDER BY recorded_on DESC", (school_id,)
    ).fetchall():
        exam_results.append({
            "id": row["id"],
            "studentId": row["student_id"],
            "studentName": student_map.get(row["student_id"], row["student_id"]),
            "term": row["term"],
            "subject": row["subject"],
            "score": row["score"],
            "grade": row["grade"],
            ...
            "recordedByName": staff_map.get(row["recorded_by"] or "", row["recorded_by"] or "-"),
    ...
    announcements = []
    for row in conn.execute(
        "SELECT * FROM sms_announcements WHERE school_id = ? ORDER BY created_at DESC", (school_id,)
    ).fetchall():
        announcements.append({
            "id": row["id"],
            "title": row["title"],
            "content": row["content"] or "",
            "targetAudience": row["target_audience"] or "all",
    ...
    return {
        "meta": {"schoolId": school_id, "schoolName": school["name"], "generatedAt": datetime.utcnow().isoformat() + "Z"},
        "staff": staff,
        "students": students,
...
class SMSRequestHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if path.startswith("/api/students/"):
            ...
            if not school:
                return
            tail = unquote(path.split("/api/students/", 1)[1])

            if tail.endswith("/profile"):
                student_id = tail[: -len("/profile")]
                with get_conn() as conn:
                    ...
                    if not student:
                        ...
                        return
                    reg_rows = conn.execute(
                        """SELECT rc.course_id, c.code, c.title, c.program, c.level, c.credits, r.term
                           FROM sms_registrations r
                           JOIN sms_registration_courses rc ON rc.registration_id = r.id
                           JOIN sms_courses c ON c.id = rc.course_id
                           WHERE r.student_id = ? ORDER BY r.term DESC, c.code""",
                        (student_id,),
                    ).fetchall()
                    courses = [
                        {"code": r["code"], "title": r["title"], "program": r["program"],
                         "level": r["level"], "credits": r["credits"], "term": r["term"]}
                        for r in reg_rows
                    ]
                    att_rows = conn.execute(
                        """SELECT a.date, c.code, c.title, ar.present
                           FROM sms_attendance_records ar
                           JOIN sms_attendance a ON a.id = ar.attendance_id
                           JOIN sms_courses c ON c.id = a.course_id
                           WHERE ar.student_id = ? AND a.school_id = ?
                           ORDER BY a.date DESC LIMIT 20""",
                        (student_id, school["id"]),
                    ).fetchall()
                    attendance = [
                        {"date": r["date"], "code": r["code"], "title": r["title"],
                         "status": "present" if r["present"] else "absent"}
                        for r in att_rows
                    ]
                    att_all = conn.execute(
                        """SELECT ar.present FROM sms_attendance_records ar
                           JOIN sms_attendance a ON a.id = ar.attendance_id
                           WHERE ar.student_id = ? AND a.school_id = ?""",
                        (student_id, school["id"]),
                    ).fetchall()
                    present_count = sum(1 for r in att_all if r["present"])
                    result_rows = conn.execute(
                        """SELECT id, recorded_on, term, subject, score, grade
                    ...
                    results = [
                        {"id": r["id"], "recordedOn": r["recorded_on"], "term": r["term"],
                         "subject": r["subject"], "score": r["score"], "grade": r["grade"]}
                ...
                self.send_json({
                    ...
                    "courses": courses,
                    "attendance": attendance,
                    "attendanceSummary": {"present": present_count, "total": len(att_all)},
                    "results": results,
                    "averageScore": avg,
                })
                return
        ...
        if path == "/api/job-applications/list":
            ...
            apps = [
                {
                    "id": r["id"], "position": r["position"], "fullName": r["full_name"],
                    "email": r["email"], "phone": r["phone"] or "", "qualification": r["qualification"] or "",
                    "experience": r["experience"], "department": r["department"] or "",
                    "cvFileName": r["cv_file_name"] or "", "notes": r["notes"] or "",
                    "status": r["status"], "submittedOn": r["submitted_on"],
                }
                for r in rows
            ]
            self.send_json({"ok": True, "jobApplications": apps})
            return
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        match = re.match(r"^/api/students/([^/]+)/toggle$", path)
        if match:
            self.handle_toggle_student(unquote(match.group(1)))
            return

        match = re.match(r"^/api/students/([^/]+)/reset-password$", path)
        if match:
            self.handle_reset_student_password(unquote(match.group(1)))
            return

        match = re.match(r"^/api/exam-results/([^/]+)/delete$", path)
        if match:
            self.handle_delete_exam_result(unquote(match.group(1)))
            return

        match = re.match(r"^/api/job-applications/([^/]+)/shortlist$", path)
        if match:
            self.handle_update_job_application(unquote(match.group(1)), "shortlisted")
            return

        match = re.match(r"^/api/job-applications/([^/]+)/reject$", path)
        if match:
            self.handle_update_job_application(unquote(match.group(1)), "rejected")
            return

        match = re.match(r"^/api/staff/([^/]+)/toggle$", path)
        if match:
            self.handle_toggle_staff(unquote(match.group(1)))
            return

        if path == "/api/registrations/admin":
            self.handle_admin_registrations(body)
            return
    ...
    def handle_admissions(self, body: dict) -> None:
        # Accept schoolId from request body as fallback to X-School-ID header
        body_school_id = str(body.get("schoolId", "")).strip()
        if body_school_id:
            with get_conn() as conn:
                school = conn.execute("SELECT * FROM sms_schools WHERE id = ?", (body_school_id,)).fetchone()
            if not school:
                self.send_json({"ok": False, "message": "School not found."}, HTTPStatus.NOT_FOUND)
                return
        else:
    ...
    def handle_add_student(self, body: dict) -> None:
        school = self.require_school()
        if not school:
            return
        ...
        password = str(body.get("password", "")).strip() or "Student@123"

        if not name or not program or not level:
            self.send_json({"ok": False, "message": "Name, program and level are required."}, HTTPStatus.BAD_REQUEST)
            return
        ...
        with get_conn() as conn:
            if email and conn.execute(
                ...
                self.send_json({"ok": False, "message": "A student with that email already exists."}, HTTPStatus.BAD_REQUEST)
                return
            ...
            conn.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?, ?)
                """,
                (student_id, school["id"], name, email, phone, program, level, password, date.today().isoformat()),
            )
    ...
    def handle_job_application(self, body: dict) -> None:
        ...
        if not school_id:
            self.send_json({"ok": False, "message": "School ID is required."}, HTTPStatus.BAD_REQUEST)
            return
        ...
        if not full_name or not email or not position:
            self.send_json({"ok": False, "message": "Full name, email and position are required."}, HTTPStatus.BAD_REQUEST)
            return
        ...
        with get_conn() as conn:
            ...
            job_id = next_id(conn, "sms_job_applications", "JOB-")
            ...
            conn.execute(
                """INSERT INTO sms_job_applications
                   (id, school_id, school_name, position, full_name, dob, email, phone, address,
                    qualification, experience, department, cv_file_name, notes, status, submitted_on)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?)""",
                (
                    job_id, school_id, school["name"], position, full_name,
                    str(body.get("dob", "")),
                    email,
                    str(body.get("phone", "")).strip(),
                    str(body.get("address", "")).strip(),
                    str(body.get("qualification", "")).strip(),
                    int(body.get("experience", 0) or 0),
        ...
        self.send_json({"ok": True, "message": "Job application submitted.", "jobApplicationId": job_id})


    def handle_toggle_student(self, student_id: str) -> None:
        school = self.require_school()
        if not school:
            return
        with get_conn() as conn:
            row = conn.execute(
                "SELECT status FROM sms_students WHERE id = ? AND school_id = ?",
                (student_id, school["id"]),
            ).fetchone()
            if not row:
                self.send_json({"ok": False, "message": "Student not found."}, HTTPStatus.NOT_FOUND)
            ...
            conn.execute("UPDATE sms_students SET status = ? WHERE id = ?", (new_status, student_id))
        self.send_json({"ok": True, "status": new_status, "message": f"Student {new_status}."})

    def handle_reset_student_password(self, student_id: str) -> None:
        school = self.require_school()
        if not school:
            return
        with get_conn() as conn:
            ...
            if not row:
                self.send_json({"ok": False, "message": "Student not found."}, HTTPStatus.NOT_FOUND)
                return
            conn.execute("UPDATE sms_students SET password = ? WHERE id = ?", ("Student@123", student_id))
        self.send_json({"ok": True, "message": "Password reset to Student@123."})

    def handle_delete_exam_result(self, result_id: str) -> None:
        school = self.require_school()
        if not school:
            return
        with get_conn() as conn:
            row = conn.execute(
                "SELECT id FROM sms_exam_results WHERE id = ? AND school_id = ?",
                (result_id, school["id"]),
            ).fetchone()
            if not row:
                self.send_json({"ok": False, "message": "Result not found."}, HTTPStatus.NOT_FOUND)
                return
            conn.execute("DELETE FROM sms_exam_results WHERE id = ?", (result_id,))
        self.send_json({"ok": True, "message": "Result deleted."})

    def handle_update_job_application(self, job_id: str, new_status: str) -> None:
        school = self.require_school()
        if not school:
            return
        with get_conn() as conn:
            row = conn.execute(
                "SELECT id FROM sms_job_applications WHERE id = ? AND school_id = ?",
                (job_id, school["id"]),
            ).fetchone()
            if not row:
                self.send_json({"ok": False, "message": "Application not found."}, HTTPStatus.NOT_FOUND)
                return
            conn.execute("UPDATE sms_job_applications SET status = ? WHERE id = ?", (new_status, job_id))
        self.send_json({"ok": True, "status": new_status, "message": f"Application {new_status}."})

    def handle_toggle_staff(self, staff_id: str) -> None:
        school = self.require_school()
        if not school:
            return
        with get_conn() as conn:
            row = conn.execute(
                "SELECT status FROM sms_staff WHERE id = ? AND school_id = ?",
                (staff_id, school["id"]),
            ).fetchone()
            if not row:
                self.send_json({"ok": False, "message": "Staff not found."}, HTTPStatus.NOT_FOUND)
                return
            new_status = "suspended" if row["status"] == "active" else "active"
            conn.execute("UPDATE sms_staff SET status = ? WHERE id = ?", (new_status, staff_id))
        self.send_json({"ok": True, "status": new_status, "message": f"Staff {new_status}."})

    def handle_admin_registrations(self, body: dict) -> None:
        school = self.require_school()
        if not school:
            return
        student_id = str(body.get("studentId", "")).strip()
        term = str(body.get("term", "")).strip()
        course_ids = body.get("courseIds", [])
        if not student_id or not term or not course_ids:
            self.send_json(
                {"ok": False, "message": "Student, term and at least one course are required."},
                HTTPStatus.BAD_REQUEST,
            )
            return
        with get_conn() as conn:
            student = conn.execute(
                "SELECT id, name FROM sms_students WHERE id = ? AND school_id = ?",
                (student_id, school["id"]),
            ).fetchone()
            if not student:
                self.send_json({"ok": False, "message": "Student not found."}, HTTPStatus.NOT_FOUND)
            ...
            reg_id = next_id(conn, "sms_registrations", "REG-")
            conn.execute(
                "INSERT INTO sms_registrations (id, school_id, student_id, term, registered_on) VALUES (?, ?, ?, ?, ?)",
                (reg_id, school["id"], student_id, term, date.today().isoformat()),
            )
            registered = 0
            for course_id in course_ids:
                if conn.execute(
                    "SELECT 1 FROM sms_courses WHERE id = ? AND school_id = ?",
                    (course_id, school["id"]),
                ).fetchone():
                    conn.execute(
                        "INSERT OR IGNORE INTO sms_registration_courses (registration_id, course_id) VALUES (?, ?)",
                        (reg_id, course_id),
                    )
                    registered += 1
        self.send_json({
            "ok": True,
            "message": f"{student['name']} registered for {registered} course(s) in {term}.",
